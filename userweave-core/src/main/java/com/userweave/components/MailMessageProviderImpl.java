...
public abstract class MailMessageProviderImpl implements MailMessageProvider {
	...
	@Override
	public final String toUrl(String token, Class<? extends WebPage> page) 
	{
		String url = getUrl(component, token, Session.get().getLocale(), page);
		// remove prefix "../"
		while(url.contains("../")) 
		{
	...
	public String getUrl(Component component, String token, Locale locale, Class<? extends WebPage> page) {
		...
		return RequestCycle.get().getUrlRenderer().renderFullUrl(
				   Url.parse(component.urlFor(page,parameters).toString()));
	}
